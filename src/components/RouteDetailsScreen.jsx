...
function RouteDetailsScreen({ route, routeTarget }) {
  ...
  const destinationAddress = routeTarget?.destinationAddress ?? route.stationAddress ?? ''
  ...
  const steps = routeTarget?.destinationCoords
    ? [
        ...
        { step: '02', title: `Head toward ${destination}`, subtitle: destinationAddress || `About ${destinationDistance}` },
        { step: '03', title: 'Arrive at destination', subtitle: `${destination} · ${destinationDistance}` },
      ]
    : [
        { step: '01', title: 'Start from current location', subtitle: originLabel },
        { step: '02', title: 'Follow the main safety corridor', subtitle: 'Use well-lit streets and main junctions' },
        { step: '03', title: 'Reach nearest police station', subtitle: `${destination} · ${destinationAddress}` },
      ]
